$(function () {
    ...
    $(".fc-month-button").prop("disabled", true);
    $(".fc-basicWeek-button").prop("disabled", false);
    ...
    $('.fc-month-button').click(function() {      // 월 변경 시에도 동일하게 적용
    ...
    $('.fc-basicWeek-button').click(function() {      // 주 변경 시에도 동일하게 적용
        $(document).ready(function() {
            $(".fc-month-button").prop("disabled", false);
            $(".fc-basicWeek-button").prop("disabled", true);
            cells = $('.fc-day').siblings();
            //예약 날짜와 달력 날짜가 같으면 예약자 표시
            for (k = 0; k < items.length; k++) {
                for (i = 0; i < cells.length; i++) {
